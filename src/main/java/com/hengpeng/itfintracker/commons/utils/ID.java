...
public class ID {
	...
	public synchronized Long getID(int idLength) {
		...
		if (idLength == 16) {
			...
			return Long.parseLong(System.currentTimeMillis() + padLeft(String.valueOf((count++)), 3, '0') + getRandom(2));
		} else if (idLength == 19) {
			return Long.parseLong(System.currentTimeMillis() + padLeft(String.valueOf((count++)), 3, '0') + getRandom(3));
		} else {
			return Long.parseLong(System.currentTimeMillis() + padLeft(String.valueOf((count++)), 3, '0') + getRandom(2));
		}
	}
